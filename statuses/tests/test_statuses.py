...
from statuses.views import (CREATE_VIEW, CREATE_TITLE, LIST_VIEW,
                            MESSAGE_CREATE_SUCCESS, LIST_TITLE, UPDATE_VIEW,
                            UPDATE_TITLE, MESSAGE_UPDATE_SUCCESS,
                            DELETE_VIEW, QUESTION_DELETE, DELETE_TITLE,
                            MESSAGE_DELETE_SUCCESS, DELETE_CONSTRAINT_MESSAGE)
...
@pytest.mark.django_db
def test_create_post(client, log_user1, setup_tasks):
...
@pytest.mark.django_db
def test_view(client, log_user1, setup_tasks):
...
@pytest.mark.django_db
def test_update_html(client, log_user1, setup_tasks):
...
@pytest.mark.django_db
def test_update_post(client, log_user1, setup_tasks):
    item = Status.objects.all().first()
    new_name = item.name + 'tests'
...
@pytest.mark.django_db
def test_delete_html(client, log_user1, setup_tasks):
    item = Status.objects.all().first()
...
@pytest.mark.django_db
def test_delete_unbound(client, log_user1, setup_tasks):
...
@pytest.mark.django_db
def test_delete_bound(client, log_user1, setup_tasks):
    count_before = Status.objects.all().count()
